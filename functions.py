...
async def get_info_from_sms(input_string):
    print(input_string)
    input_string2 = input_string.split("отправил(а)")
    input_string2 = input_string2[1].split("). оплатил(а) ваш счёт")[1]
    print(input_string2)
    # Извлечение суммы после $
    amount_matches = re.findall(r'\$\s*(\d+(\.\d+)?)', input_string2)
    if amount_matches:
        amounts = [float(match[0]) for match in amount_matches]
    else:
        amounts = None
    # Извлечение суммы между скобками
    bracket_matches = re.findall(r'\(\s*(\d+(\.\d+)?)\s*\)', input_string2)
    if bracket_matches:
...
def apply_percentage(number, percentage):
    if number is not None and percentage is not None:
        print(f'num: {number}, perc: {percentage}')
